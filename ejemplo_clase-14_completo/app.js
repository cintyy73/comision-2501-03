...
// Referencia al cuerpo de la tabla donde se mostrarán los estudiantes
...
//  DELETE
// Elimina un usuario por su ID, tras confirmación
...
// Variable para saber si estamos editando (id) o creando (null)
let editUserId = null;

// Muestra el formulario y cambia el texto del botón según el modo
function mostrarForm(modo = "crear") {
  form.style.display = "block";
  btnGuardar.textContent = modo === "editar" ? "Actualizar" : "Guardar";
}

// Oculta el formulario y reinicia el estado
...
// Evento para mostrar el formulario vacío al crear usuario
btnAbrirCrear.addEventListener("click", () => {
  form.reset();
  editUserId = null;
  mostrarForm("crear");
});
// Evento para ocultar el formulario al cancelar
...
// Evento para guardar (crear o editar) usuario al enviar el formulario
form.addEventListener("submit", async (e) => {
  e.preventDefault();
  const name = document.getElementById("nombre").value;
  const country = document.getElementById("pais").value;
...
// Inicializa la app mostrando los usuarios al cargar
obtenerUsuarios();
